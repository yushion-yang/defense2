...
func TestBuffTemplate_AllHaveID(t *testing.T) {
	templates := enemy.GetBuffTemplates()
	for id, tmpl := range templates {
		if tmpl.ID == "" {
			t.Errorf("buff 模板 %q 的 ID 为空", id)
		}
		if tmpl.ID != id {
			t.Errorf("buff 模板 key=%q 但 ID=%q 不一致", id, tmpl.ID)
		}
	}
}

func TestBuffTemplate_AllHaveCategory(t *testing.T) {
	templates := enemy.GetBuffTemplates()
	validCats := map[string]bool{"offense": true, "defense": true, "utility": true, "death": true}
	for id, tmpl := range templates {
		if !validCats[tmpl.Category] {
			t.Errorf("buff 模板 %q category=%q 不合法", id, tmpl.Category)
		}
	}
}

func TestBuffTemplate_BerserkHasThreshold(t *testing.T) {
	tmpl := enemy.GetBuffTemplate("berserk")
	if tmpl == nil {
		t.Fatal("berserk 模板不存在")
	}
	if tmpl.BerserkThreshold <= 0 || tmpl.BerserkThreshold >= 1 {
		t.Errorf("berserk threshold=%.2f 应在 (0, 1)", tmpl.BerserkThreshold)
	}
	if tmpl.BerserkSpeedScale <= 1 {
		t.Errorf("berserk speedScale=%.2f 应 > 1", tmpl.BerserkSpeedScale)
	}
}

func TestBuffTemplate_RegenHasRate(t *testing.T) {
	tmpl := enemy.GetBuffTemplate("regen")
	if tmpl == nil {
		t.Fatal("regen 模板不存在")
	}
	if tmpl.RegenPerSec <= 0 {
		t.Errorf("regen regenPerSec=%.4f 应 > 0", tmpl.RegenPerSec)
	}
}
