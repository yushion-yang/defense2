...
// 验证 classic-waves.json 的完整性和一致性。
...
// TestClassicWavesLoad 验证经典出怪配置加载成功。
func TestClassicWavesLoad(t *testing.T) {
	cwc, err := config.LoadClassicWavesConfig()
	if err != nil {
		t.Fatalf("LoadClassicWavesConfig() 失败: %v", err)
	}
	if cwc.TotalWaves <= 0 {
	...
	for _, entry := range cwc.Waves {
		for i, ee := range entry.Enemies {
			if _, ok := archetypes[ee.Archetype]; !ok {
				t.Errorf("wave %d enemies[%d]: 原型 %q 不存在于 enemies-core.json", entry.Wave, i, ee.Archetype)
			}
		}
		if entry.Boss != nil {
			if _, ok := archetypes[entry.Boss.Archetype]; !ok {
				t.Errorf("wave %d boss: 原型 %q 不存在于 enemies-core.json", entry.Wave, entry.Boss.Archetype)
			}
		}
	}
}
	...
	for _, entry := range cwc.Waves {
		...
		for idxStr, buffIDs := range entry.Buffs {
			...
			for _, bid := range buffIDs {
				if abilTable != nil {
					if abilTable[bid] == nil {
						t.Errorf("wave %d buffs[%d]: buff ID %q 不存在于 abilities.json", entry.Wave, idx, bid)
					}
				}
			}
		}
...
// TestClassicWavesScalingValid 验证缩放参数合理。
