...
| A7 | HP 基础公式 | 读 spawner.Tick | `baseHP = (10 + wave*5) * HPScale` |
| A8 | Speed 基础公式 | 同上 | `baseSpeed = (50 + wave*3) * SpeedScale` |
| A9 | ManualWave | 读 Tick | ManualWave=true 时倒计时到 0 不自动开波 |
| A10 | AllDone 标记 | 读 Tick | wave >= maxWaves 时设 AllDone=true |
...
- Spawner.Tick ← stage.updatePlaying step 1（每帧驱动）
