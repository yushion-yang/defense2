...
### 5. 精灵动画 ✅
- ~~Boss 呼吸缩放（周期性 Scale 变化）~~ — Boss 1.0+0.04*sin(t*1.8), Elite 1.0+0.02*sin(t*2.2)
- ~~战灵 idle 悬浮动画（上下浮动 + 缩放脉冲）~~ — bobY=sin(t*2.5)*3 + scale 1.0+0.015*sin(t*2.0)
...
### 9. 伤害数字（暴击已贯通，治疗待做）
- ~~暴击红色~~ — HitCallback 扩展 crit bool 参数，HitResult.IsCrit 贯通到 SpawnDamageText
- 治疗绿色区分（待有治疗机制时实现）
