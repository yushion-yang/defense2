...
类比炮塔 6 类别各选 1 个能力，怪物同样设计为 **6 类别能力装配**：

```
怪物 = 基础原型 (HP/速度/半径) + 6 能力槽 (各类别 0~1 个)
```
...
| 类别 | 目的 | 对应炮塔克制 |
|------|------|-------------|
| **defense (防御)** | 如何承受伤害 | 克制特定攻击方式 |
| **resist (抗性)** | 如何抵抗 CC/DoT | 克制控制和持续伤害 |
| **movement (移动)** | 如何穿越防线 | 克制固定阵位策略 |
| **offense (攻击)** | 如何反击玩家 | 惩罚特定操作模式 |
| **support (支援)** | 如何帮助友方 | 削弱集火/单体策略 |
| **death (死亡)** | 死后留什么 | 惩罚无脑击杀 |

#### 克制关系矩阵

```
            弹射物  光束  AoE  CC  DoT  爆发  多段  区域
弹幕盾      ██      --   --   --  --   --    --   --    ← 只挡弹射物
装甲        ██      ░░   ░░   --  --   --    ██   --    ← 克多段,被爆发克
闪避        ██      ██   --   ██  --   ██    --   --    ← 被多段克
坚韧        --      --   --   --  --   ██    --   --    ← 被沉默克
...
| 怪物预设 | 原型 | 能力组合 | 设计意图 |
|---------|------|---------|---------|
| 铁壁卫士 | tank | 弹幕盾+意志坚定+援护 | 前排坦克,挡弹+免CC+分摊伤害 |
| 暗影刺客 | runner | 闪避+受击冲刺+自爆 | 高速突入,打不中+越打越快,死了还炸 |
| 毒抗突击 | armored | 装甲+毒抗+加速光环 | 克制DoT流+减速流 |
| 经济掠夺 | normal | 蚀金+净化+诅咒 | 经济战,被打扣金,死了也扣金 |
| 相位法师 | normal | 相位偏移+治疗光环+反伤 | 周期无敌+回血+反伤,逼玩家用多种策略 |
| Boss: 军团统帅 | tank+boss | 坚韧+意志坚定+削弱光环+加速光环+分裂 | 全面压制,死后还分裂 |
