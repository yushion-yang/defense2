...
var (
	...
	ToneGold      = rgba(255, 193, 7, 255)  // 金色（星级评价等）
)
...
var (
	BeamHighlight = hex(0xffffff) // 光束白色高光 RGB（alpha 动态）
)
...
const (
	MapPathDashDarken float64 = 0.7 // 路径虚线暗化因子
	MapDecoAlphaA     uint8   = 25  // 地形装饰 alpha 层 A
	MapDecoAlphaB     uint8   = 20  // 地形装饰 alpha 层 B
	MapDecoAlphaC     uint8   = 15  // 地形装饰 alpha 层 C
)
