...
type AbilityDef struct {
	...
	Param2    float64 `json:"param2"`    // 第二固定参数（0=无，部分能力需要两个固定参数）
	Param2Dim string  `json:"param2Dim"` // 第二固定参数的含义标识
	Display   string  `json:"display"`   // HUD 展示模板（{s}=缩放值 {s%}=缩放百分比 {si}=缩放取整 {sh%}=缩放减半百分比 {p}=参数 {p%}=参数百分比 {p2}=参数2 {p2%}=参数2百分比）
}
